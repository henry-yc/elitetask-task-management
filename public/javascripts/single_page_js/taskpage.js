...
// Hide the more options menu when user clicks away
document.addEventListener("click", function(event) {
    // If user clicks inside the element, do nothing
	if (event.target.closest(".more-options")) return;
	// If user clicks ellipsis, do nothing also (otherwise clicking the button also hides div so it never opens)
	if (event.target.closest(".ellipsis-button")) return;
	// Else hide all open menus on any click
...
function addCalendarEvent() {
    var event = {
        ...
        'start': {
            'dateTime': '2020-06-21T20:00:00',
            'timeZone': 'Australia/Adelaide'
        },
        'end': {
            'dateTime': '2020-06-21T20:01:00',
            'timeZone': 'Australia/Adelaide'
        },
        'attendees': [{
                'email': 'a1647362@student.adelaide.edu.au'
            },
            {
                'email': 'fghfgh@example.com'
            }
        ],
        'reminders': {
            'useDefault': false,
...
function updateCalendarEvent(eventId) {
    ...
    gapi.client.calendar.events.get({
        ...
        // Update event
        event.attendees = ['a1647362@student.adelaide.edu.au'];
...
var vue = new Vue ({
    ...
    data: {
        // currentUser: {userid: 2, firstName: "Henry", lastName: "Yin-Chen", avatar: "23e092379ad791e387f491e79f1a17db", isManager: true},
        ...
        membersToRemove: [],
        // members: [{userid: 2, name: "Henry Yin-Chen"}, {userid: 3, name: "James Dean"}, {userid: 4, name: "Lucy Lu"}],
        members: [],
        // taskTypes: ["Brainstorming", "Coding", "Cleaning", "Debugging", "Documentation", "Graphic Design", "Housekeeping", "Lecture", "Management", "Paperwork", "Proofing"],
        taskTypes: [],
        nightmode: false,
        // nightmodeSwitch: this.nightmode,
        currentTaskId: "",
    ...
    methods: {
        ...
        setNightmodeCookie: function() {
            var d = new Date();
            d.setTime(d.getTime() + (30*24*60*60*1000)); // Set expiry 30 days from now
            var expires = "expires="+ d.toUTCString();
            document.cookie = "nightmode=" + this.nightmode + ";" + expires + ";path=/; secure";
        },
